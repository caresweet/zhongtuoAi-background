...
def _get_chapter_specific_requirements(num: int) -> str:
    ...
    if num == 1:
        return """
## 📐 第一章特定要求（专家反复指出，必须遵守）
- 1.3 稳评责任单位：必须明确指出责任单位是谁（写"稳评责任单位：XXX"，XXX 取「可用数据」的责任单位/org_name）
- 1.4 稳评实施单位：必须明确指出实施单位是谁（写"稳评实施单位：当地政府 + 江苏众拓项目代理咨询有限公司"，两个名称都要有）
- 征收目的（1.5.3 等小节）：段落开头**不要出现"的规定"这三个字**，直接陈述征收目的（如"本次拟征收土地用于……"）
- 表1-1 拟征收土地基本情况表：放在「征收面积」小节（1.5.2）的末尾，不要放在其他位置
- 资金保障小节（1.7.3）：**不要放表格**，用文字描述资金保障情况即可
"""
    if num == 2:
        return """
## 📐 第二章特定要求（专家反复指出，必须遵守）
